...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create trainplog
  fly apps create trainplog-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app trainplog
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app trainplog-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'trainplog-staging' does not match app name in config file 'trainplog'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name trainplog-db
  fly postgres attach --app trainplog trainplog-db

  fly postgres create --name trainplog-staging-db
  fly postgres attach --app trainplog-staging trainplog-staging-db
  ```
